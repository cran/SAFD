...
function(XX,V,theta=1/3,B=1000,pic=1){
 ...
 if(nrow(temp_sum)>1){
  ...
  plot(sample_mean,type="l", xlim=limx,lwd=2,xlab=NA, ylab=expression(alpha),cex.main=1, lty="dashed",
           main=paste("Sample mean (dashed line) and V (solid line)",sep=""))
  ...
  if(pic==1){
   ...
   abline(a = NULL, b = NULL, v = test_statistic,col="black",lty="dotted",lwd=3)
   TS<-test_statistic
